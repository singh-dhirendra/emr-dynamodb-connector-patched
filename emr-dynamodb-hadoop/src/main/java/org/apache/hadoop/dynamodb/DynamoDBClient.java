...
import static org.apache.hadoop.dynamodb.DynamoDBConstants.DYNAMODB_SESSION_TOKEN;
...
public class DynamoDBClient {
  ...
  public TableDescription describeTable(String tableName) {
    ...
    try {
      ...
      return describeResult.result.getTable();
        ...
        String s = DynamoDBClient.class.getResource("DynamoDBClient.class").toString();
        String s1 = config.get("dynamo.crossAccountRole");
        throw new RuntimeException("Could not lookup table " + tableName + " in DynamoDB." + s + "  s1:" + s1, e);
    }
  }
  ...
  public RetryResult<ScanResult> scanTable(
      ...
      final String filterKey = config.get("scan.filter");
    ...
    if(!Strings.isNullOrEmpty(filterKey)){
      ...
      scanFilter.put("key", new Condition().withAttributeValueList(new AttributeValue().withS(filterKey)).withComparisonOperator(ComparisonOperator.CONTAINS));
  ...
  protected AWSCredentialsProvider getAWSCredentialsProvider(Configuration conf) {
    ...
    String secretKey = conf.get(DYNAMODB_CREDENTIAL_PAIR_NAME.getSecretKeyName());
    String crossAccountRole = conf.get("dynamo.crossAccountRole");

    if(!Strings.isNullOrEmpty(crossAccountRole)){

      AWSSecurityTokenService stsClient = AWSSecurityTokenServiceClient.builder()
            .withCredentials(InstanceProfileCredentialsProvider.getInstance())
            .withRegion(conf.get(DynamoDBConstants.REGION))
            .build();
      log.info("[ALPS]  using crossAccountRole: " + crossAccountRole);

      return new STSAssumeRoleSessionCredentialsProvider.Builder(crossAccountRole,
            "DynamoCrossAccountRole").withStsClient(stsClient).build();
    } else if(!Strings.isNullOrEmpty(accessKey) && !Strings.isNullOrEmpty(secretKey)){
      ...
      log.info("[ALPS] using default profile: ");
      providersList.add(InstanceProfileCredentialsProvider.getInstance());
    }
